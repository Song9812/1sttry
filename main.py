    ...
    if 'user_location' in st.session_state and st.session_state['user_location']:
        ...
        if not nearby_toilets.empty:
            ...
            if selected_toilet_info:
                ...
                # 주소는 그대로 전달하면 네이버/카카오에서 인식 못할 가능성 있음. 좌표 기반으로 URL 생성
                encoded_user_address = urllib.parse.quote(user_address)
                encoded_toilet_address_name = urllib.parse.quote(selected_toilet_display_name.split('(')[0].strip()) # 건물명만 인코딩

                # 카카오맵 도보 길찾기 URL (위도, 경도 기반)
                # sX, sY: 출발지 경도, 위도
                # eX, eY: 목적지 경도, 위도
                kakao_map_url = (
                    f"https://map.kakao.com/link/to/{encoded_toilet_address_name},{selected_toilet_info['lat']},{selected_toilet_info['lon']}"
                    f"?sp={user_lat},{user_lon}&eX={selected_toilet_info['lon']}&eY={selected_toilet_info['lat']}"
                    f"&target=walk" # 도보 경로
                )
                
                # 네이버 지도로 주소 검색만 하는 URL (길찾기 대신 검색으로 단순화)
                # 네이버 지도의 길찾기 URL은 복잡한 내부 좌표를 사용하므로, 검색으로 대체합니다.
                naver_search_url = (
                    f"https://map.naver.com/p/search/{encoded_toilet_address_name}"
                    f"?c={user_lat},{user_lon},15,0,0,0,dh" # 지도 중심을 사용자 위치 근처로
                )

                # Google 지도 URL (위도, 경도 기반)
                Maps_url = (
                ...
                st.markdown(f"[**네이버 지도에서 검색하기**]({naver_search_url})", unsafe_allow_html=True)
